...
# example ./libs/bash/install.sh some/branch/name 2
# example ./libs/bash/install.sh some/branch/name 1
# usage ./libs/bash/install.sh (optional) BRANCH_NAME (optional) PROJECT_TYPE
# version: 0.9.0  - 2025-08-17 - UPDATED project source to be configurable via variable `PROJECT_SOURCE` 
# version: 0.9.0  - 2025-02-11 - ADD `PROJECT_TYPE` selection logic
# version: 0.9.0  - 2025-02-11 - UPDATED `TOOLCHAIN_DEV` logic to be easier to reason about
...
# version: 0.8.3  - 2024-10-28 - ADD ENV VAR TOOLCHAIN_DEV to enable easier localhost development
...
# version: 0.7.0  - 2024-06-19 - UPDATED Toolchain source URL post migration to GitLab SCM hosting
...
declare TOOLCHAIN_BRANCH
TOOLCHAIN_BRANCH="main"
if [[ "$1" ]]
then
    TOOLCHAIN_BRANCH="${1}"
fi
export TOOLCHAIN_BRANCH
...
declare TOOLCHAIN_SOURCE
TOOLCHAIN_SOURCE="https://github.com/davidjeddy/toolchain-management.git"
printf "INFO: Project source is %s\n" "$TOOLCHAIN_SOURCE"
...
function gitCheckout() {
    ...
    if [[ $TOOLCHAIN_DEV ]]
    then
        printf "WARN: TOOLCHAIN_DEV is set, using localhost version of Toolchain."
    else
        ...
        git clone --quiet "$TOOLCHAIN_SOURCE" "$WORKSPACE/.tmp/toolchain-management"

        # Even if main, checkout anyways
        cd "$WORKSPACE/.tmp/toolchain-management"
        git checkout "$TOOLCHAIN_BRANCH" --force
        cd "$WORKSPACE" || exit 1
    fi
...
function gitHookSelection() {
    ...
    Next expansion of the Toolchain will have different hooks for different types of projects
    Alphabetical sort
